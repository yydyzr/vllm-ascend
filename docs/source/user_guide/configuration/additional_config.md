...
| `enable_fused_mc2`                  | int  | `0`     | Fused MC2 configuration. `0` disables the fused path and `1` enables it when the model and parallel configuration support it. MegaMoe is selected from the instantiated layer capabilities: A2/A3 support W8A8/W4A8, and A5 supports W4A8 MXFP with `group_size=32`. Unsupported layer layouts keep the non-MegaMoe path. The legacy `VLLM_ASCEND_ENABLE_FUSED_MC2` environment variable is no longer supported. |
