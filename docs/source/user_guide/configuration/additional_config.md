...
| `disable_mla_decode_head_pad`       | bool | `False` | When True, MLA decode FIA keeps the native query-head count instead of padding to the next power of 2 (for example 96 stays 96 instead of 128). Prefill is unchanged. Enable only when CANN already accepts non-power-of-2 query heads. |
...
| `enable_force_eplb`                 | bool | `False` | When True, replace MoE router `topk_ids` with a deterministic round-robin expert assignment so tokens are spread evenly across EP ranks. Cannot be mixed with `eplb_config.dynamic_eplb`. |
