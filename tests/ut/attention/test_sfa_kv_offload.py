...
def _nano_select_impl(*, enable_c8: bool):
    ...
    impl._nano_metadata = SimpleNamespace(
        ...
        nano_states=torch.tensor([-2, -3], dtype=torch.int32),
        nano_active=torch.tensor([True, True]),
        nano_is_short=torch.tensor([False, True]),
    )
    return impl


def _run_nano_select(*, enable_c8: bool, query_scale=None):
    impl = _nano_select_impl(enable_c8=enable_c8)
    if enable_c8:
        # Match indexer C8 output: flattened int8 [T*H, 128] + scale [T*H].
...
def test_indexer_passes_c8_query_scale_to_nano_topk_selector():
    ...
    indexer.forward_k = MagicMock(return_value=(torch.zeros((_LIM_T, _LIM_HEAD_DIM)), None))
