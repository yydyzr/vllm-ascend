...
struct BlockMmad<MmadAtlasA2W4A4MatmulPerChannelDequant<PRELOAD_STAGES_, L1_STAGES_, L0A_STAGES_, L0B_STAGES_,
...
public:
    // Type Aliases
    using DispatchPolicy = MmadAtlasA2W4A4MatmulPerChannelDequant<PRELOAD_STAGES_, L1_STAGES_, L0A_STAGES_, L0B_STAGES_,
                                                                  L0C_STAGES_, ENABLE_UNIT_FLAG_, ENABLE_SHUFFLE_K_>;
    ...
    using LayoutL1Scale = layout::VectorLayout;
    using LayoutFpBuf = layout::VectorLayout;
    ...
    // L1 tile size
    static constexpr uint32_t L1A_TILE_SIZE = L1TileShape::M * L1TileShape::K * sizeof(int8_t) / 2;
    static constexpr uint32_t L1B_TILE_SIZE = L1TileShape::N * L1TileShape::K * sizeof(int8_t) / 2;
    static constexpr uint32_t SCALE_KTILE_SIZE = L1TileShape::N * sizeof(uint64_t);
    // L0 tile size
    static constexpr uint32_t L0A_TILE_SIZE = L0TileShape::M * L0TileShape::K * sizeof(int8_t) / 2;
    static constexpr uint32_t L0B_TILE_SIZE = L0TileShape::K * L0TileShape::N * sizeof(int8_t) / 2;
    ...
    // Check L1TileShape
    static_assert((L1A_TILE_SIZE + L1B_TILE_SIZE + SCALE_KTILE_SIZE) * L1_STAGES <= ArchTag::L1_SIZE,
    ...
    CATLASS_DEVICE
    void operator()(AscendC::GlobalTensor<uint64_t> const &gmScale, LayoutScale const &layoutScale,
                    ...
                    Callback const &callbackAfterFixpipe, int32_t syncLoopIdx = -1, int32_t flag = 0)
    {
        ...
        // Copy scale from GM to L1
        auto layoutTileScale = layoutScale.GetTileLayout(MakeCoord(L1TileShape::N));
        LayoutL1Scale layoutL1KTileScale{L1TileShape::N};
        AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(l1ScaleEventList[l1ScaleListId]);
        copyGmToL1Scale(l1ScaleBufList[l1ScaleListId], gmScale, layoutL1KTileScale, layoutTileScale);
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; ++kLoopIdx) {
            ...
            uint32_t kActual = (kTileIdx < kTileCount - 1) ? L1TileShape::K :
                                                             (actualShape.k() - kTileIdx * L1TileShape::K);
            ...
            if (kLoopIdx == kTileCount - 1) {
                l1TileMmadParams.l1ScaleListId = l1ScaleListId;
        ...
        l1ScaleListId = (l1ScaleListId + 1 < L1_STAGES) ? (l1ScaleListId + 1) : 0;
    }
    ...
    void Finalize(int32_t target, int32_t flag = 0)
    {
        if (ptrSoftFlagBase_ != nullptr) {  // 软同步方案
            ...
            flagGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(ptrSoftFlagBase_) +
...
private:
    ...
    void InitL1(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart)
    {
        ...
        uint32_t l1ScaleOffset = l1BOffset + L1B_TILE_SIZE * L1_STAGES;
        for (uint32_t i = 0; i < L1_STAGES; ++i) {
            ...
            l1ScaleBufList[i] = resource.l1Buf.template GetBufferByByte<uint64_t>(l1ScaleOffset + SCALE_KTILE_SIZE * i);
            l1AEventList[i] = i;
            l1BEventList[i] = i + L1_STAGES;
            l1ScaleEventList[i] = i;
        ...
        if (ptrSoftFlagBase_ != nullptr) {
            ...
            // 16 0 0 0 0 0 0 0
            // 搬到L1
            uint32_t l1FOffset = l1ScaleOffset + SCALE_KTILE_SIZE * L1_STAGES;
    ...
    void L1TileMmad(L1TileMmadParams const &params)
    {
        ...
        LayoutFpBuf layoutFpBuf{L1TileShape::N};
        LayoutL1Scale layoutL1Scale{L1TileShape::N};
            ...
            for (uint32_t kPartIdx = 0; kPartIdx < kPartLoop; ++kPartIdx) {
                ...
                for (uint32_t nPartIdx = 0; nPartIdx < nPartLoop; ++nPartIdx) {
                    uint32_t nPartActual = (nPartIdx < nPartLoop - 1) ? L0TileShape::N :
                                                                        (params.nRound - nPartIdx * L0TileShape::N);
        ...
        if (params.isKLoopLast) {
            ...
            if constexpr (!ENABLE_UNIT_FLAG) {
                ...
                // Copy scale from L1 to FixpipeBuffer
                auto &l1ScaleBuf = l1ScaleBufList[params.l1ScaleListId];
                AscendC::WaitFlag<AscendC::HardEvent::MTE2_FIX>(params.l1ScaleListId);
                copyL1ToFP(fixpipeBuf, l1ScaleBuf, layoutFpBuf, layoutL1Scale);
...
#endif  // CATLASS_GEMM_BLOCK_BLOCK_MMAD_W4A4_PERCHANNEL_HPP
