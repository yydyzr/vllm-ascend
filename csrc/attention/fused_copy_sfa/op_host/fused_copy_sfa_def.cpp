...
class FusedCopySfa : public OpDef {
public:
    explicit FusedCopySfa(const char *name) : OpDef(name)
    {
        ...
        this->Input("dram_block_table").ParamType(REQUIRED).DataTypeList({ge::DT_INT32})
            .FormatList({ge::FORMAT_ND}).AutoContiguous();
        this->Input("source_token_ids").ParamType(REQUIRED).DataTypeList({ge::DT_INT32})
            .FormatList({ge::FORMAT_ND}).AutoContiguous();
