...
/*!
 * \file sparse_tail_attention_kernel_mla.h
 * \brief
 */
...
#include "sparse_tail_attention_common.h"
#include "sparse_tail_attention_service_cube_mla.h"
#include "sparse_tail_attention_service_vector_mla.h"
...
public:
    ...
    __aicore__ inline SparseTailAttentionMla(){};
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                ...
                                const SparseTailAttentionTilingDataMla *__restrict tiling,
...
private:
    ...
    const SparseTailAttentionTilingDataMla *__restrict tilingData = nullptr;
                                                                            ...
                                                                            uint32_t n2Idx)
{
    ...
    const int32_t cacheTokenCount = cacheTokensGm.GetValue(bIdx);
    if (cacheTokenCount == 0) {
        ...
        tempLoopInfo.tailTokenCount = static_cast<uint32_t>(visibleKvLen - cacheTokenCount);
...
__aicore__ inline void SparseTailAttentionMla<STAT>::ComputeMm1(const RunInfo &info)
{
    ...
    for (uint32_t i = 0; i < nBufferLoopTimes; i++) {
        ...
        CrossCoreSetFlag<ConstInfo::STA_SYNC_MODE2, PIPE_FIX>(constInfo.syncC1V1);
    }
}
...
__aicore__ inline void SparseTailAttentionMla<STAT>::ComputeMm2(const RunInfo &info)
{
    ...
    for (uint32_t i = 0; i < nBufferLoopTimes; i++) {
        ...
        CrossCoreSetFlag<ConstInfo::STA_SYNC_MODE2, PIPE_FIX>(constInfo.syncC2V2);
        CrossCoreSetFlag<ConstInfo::STA_SYNC_MODE2, PIPE_FIX>(constInfo.syncC2V1);
    }
}
    ...
    if (extraInfo0.isValid) {
        if ASCEND_IS_AIC {
            ...
            if constexpr (TEMPLATE_MODE == V_TEMPLATE) {
                ...
                CrossCoreSetFlag<ConstInfo::STA_SYNC_MODE2, PIPE_MTE3>(constInfo.syncV0C1);
            }
        }
    }
