...
/*!
 * \file fused_sparse_attention_overlap_common.h
 * \brief
 */

#ifndef FUSED_SPARSE_ATTENTION_OVERLAP_COMMON_H
#define FUSED_SPARSE_ATTENTION_OVERLAP_COMMON_H
...
// 将isCheckTiling设置为false, 输入输出的max&sum&exp的shape�?m, 1)
constexpr SoftmaxConfig FUSED_SPARSE_ATTENTION_OVERLAP_SOFTMAX_FLASHV2_CFG_WITHOUT_BRC = {false, 0, 0, SoftmaxMode::SOFTMAX_OUTPUT_WITHOUT_BRC};

enum class FusedSparseAttentionOverlapLayout
{
    BSND = 0,
    TND = 1,
    PA_BSND = 2,
...
template <typename Q_T, typename KV_T, typename OUT_T, const bool FLASH_DECODE = false,
	  FusedSparseAttentionOverlapLayout LAYOUT_T = FusedSparseAttentionOverlapLayout::BSND, FusedSparseAttentionOverlapLayout KV_LAYOUT_T = FusedSparseAttentionOverlapLayout::BSND,
          const int TEMPLATE_MODE = C_TEMPLATE, typename... Args>
struct FusedSparseAttentionOverlapType {
    using queryType = Q_T;
    using kvType = KV_T;
    using outputType = OUT_T;
    static constexpr bool flashDecode = FLASH_DECODE;
    static constexpr FusedSparseAttentionOverlapLayout layout = LAYOUT_T;
    static constexpr FusedSparseAttentionOverlapLayout kvLayout = KV_LAYOUT_T;
    static constexpr int templateMode = TEMPLATE_MODE;
    static constexpr bool pageAttention = (KV_LAYOUT_T == FusedSparseAttentionOverlapLayout::PA_BSND);
};

// ================================Util functions==================================
template <typename T> __aicore__ inline T FusedSparseAttentionOverlapAlign(T num, T rnd)
{
...
struct ConstInfo {
    // CUBE与VEC核间同步的模�?
    static constexpr uint32_t FUSED_SPARSE_ATTENTION_OVERLAP_SYNC_MODE2 = 2;
    // BUFFER的字节数
    ...
    uint32_t splitKVNum = 0U;         // S2核间切分的切分份�?
    FusedSparseAttentionOverlapLayout outputLayout;          // 输出的Transpose格式
    uint32_t sparseMode = 0;
    bool needInit = false;

    // FlashDecoding
    uint32_t actualCombineLoopSize = 0U; // FlashDecoding场景, S2在核间切分的最大份�?
    uint64_t combineLseOffset = 0ULL;
    uint64_t combineAccumOutOffset = 0ULL;

    uint32_t actualLenDimsQ = 0U;        // query的actualSeqLength 的维�?
    uint32_t actualLenDimsKV = 0U;       // KV 的actualSeqLength 的维�?

    // TND
    uint32_t s2Start = 0U;               // TND场景下，S2的起始位�?
    ...
    uint32_t tndFDCoreArrLen = 0U;       // TNDFlashDecoding相关分核信息array的长�?
...
#endif // FUSED_SPARSE_ATTENTION_OVERLAP_COMMON_H