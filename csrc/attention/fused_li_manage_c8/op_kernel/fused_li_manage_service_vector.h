...
template <typename LIT, typename MM1_OUT_T = float>
class LIVector {
public:
    ...
    __aicore__ inline void ProcessVec0(const LICommon::RunInfo &info);
    ...
    __aicore__ inline void InitC8GlobalTensor(GlobalTensor<half> queryScaleGm, GlobalTensor<half> keyScaleGm,
                                              ...
                                              uint32_t maxBlockNumPerBatch, uint32_t blockSize);
...
protected:
    ...
    // C8-only scale tensors and block-table metadata.
...
private:
    ...
    // C8-only UB scratch for per-token key scale and combined q/weight scale.
    TBuf<TPosition::VECCALC> keyScaleBuf_;
    TBuf<TPosition::VECCALC> c8ScaleBuf_;
    ...
    __aicore__ inline void WriteMissCount(uint32_t bIdx, int32_t missCount,
                                          const LocalTensor<int32_t> &scalarLocal);
    // C8-only helpers
    __aicore__ inline void LoadKeyScaleChunk(const LICommon::RunInfo &info, const LocalTensor<half> &dst);
};

template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::InitBuffers(TPipe *pipe)
{
    if ((GetBlockIdx() & 1U) != 0) {
        return;
    }
    ...
    if constexpr (LIT::IS_C8) {
        pipe->InitBuffer(keyScaleBuf_, S2_BASE_SIZE * (sizeof(half) + sizeof(float)));
        pipe->InitBuffer(c8ScaleBuf_, GROUP_INNER * sizeof(float));
    }
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::InitMtpBuffers(TPipe *pipe)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::InitParams(
    uint32_t kSeqSize, uint32_t qHeadNum, uint32_t cacheSlotsSize)
{
...
LIVector<LIT, MM1_OUT_T>::InitVec1GlobalTensor(GlobalTensor<MM1_OUT_T> mm1ResGm, GlobalTensor<K_T> weightsGm,
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void
LIVector<LIT, MM1_OUT_T>::InitC8GlobalTensor(GlobalTensor<half> queryScaleGm, GlobalTensor<half> keyScaleGm,
                                              GlobalTensor<int32_t> blockTableGm,
                                              GlobalTensor<half> weightWorkspaceGm,
                                              uint32_t maxBlockNumPerBatch, uint32_t blockSize)
{
    this->queryScaleGm = queryScaleGm;
    this->keyScaleGm = keyScaleGm;
    this->blockTableGm_ = blockTableGm;
    this->weightWorkspaceGm = weightWorkspaceGm;
    this->maxBlockNumPerBatch_ = maxBlockNumPerBatch;
    this->blockSize_ = blockSize;
}

template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::InitMtpGlobalTensor(
    GlobalTensor<MM1_OUT_T> mm1ResGm, GlobalTensor<K_T> weightsGm,
    GlobalTensor<int32_t> cacheSlotsGm,
    GlobalTensor<int32_t> topkSlotsGm,
    GlobalTensor<int32_t> topkSourceIdsGm,
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::InitPartialMetadata(uint32_t coreIdx)
{
    if ((GetBlockIdx() & 1U) != 0) {
        return;
    }
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::WriteZeroMissCount(uint32_t bIdx)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::WriteMtpZeroMissCount(uint32_t bIdx)
{
    WriteZeroMissCount(bIdx);
}

template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::StartPayloadCopy(LocalTensor<int32_t> &payloadLocal, uint32_t cacheRowIdx,
                                                       int32_t s2BaseIdx, int32_t validLen, int32_t alignedLen)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::FinishPayload(LocalTensor<int32_t> &payloadLocal, int32_t s2BaseIdx,
                                                    int32_t validLen)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::PrepareSortScore(
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::SortTopKBySlot(const LocalTensor<float> &pairLocal,
                                                     const LocalTensor<float> &keyLocal,
                                                     const LocalTensor<int32_t> &payloadBase,
                                                     LocalTensor<float> &tmpSortBuf, bool hasLongIndexTag)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::WriteScoreChunk(uint32_t bIdx, int32_t s2BaseIdx,
                                                      const LocalTensor<float> &scoreLocal, int32_t alignedLen)
{
    SetWaitFlag<HardEvent::V_MTE3>(HardEvent::V_MTE3);
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::WriteMtpAggregateScoreChunk(
    uint32_t bIdx, uint32_t queryIdx, int32_t s2BaseIdx,
    const LocalTensor<float> &scoreLocal, int32_t alignedLen)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::CollectMtpEvictCandidateChunk(
    const LICommon::RunInfo &info,
    const LocalTensor<int32_t> &payloadLocal,
    LocalTensor<float> &tmpSortBuf,
    uint32_t cachedChunkIdx)
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::WritePartialTopK(uint32_t coreIdx, uint32_t partialSlot, uint32_t bIdx)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::SortEvictCandidateChunk(uint32_t bIdx, uint32_t cacheRowIdx,
                                                              uint32_t chunkIdx,
                                                              uint32_t actualSeqLen,
                                                              const LocalTensor<float> &pairOut,
                                                              LocalTensor<float> &tmpSortBuf)
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::MergeEvictCandidateChunk(const LocalTensor<float> &chunkPairLocal,
                                                               uint32_t candidateCap,
                                                               LocalTensor<float> &tmpSortBuf)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::MergeEvictChunkBatch512(uint32_t bIdx, uint32_t cacheRowIdx,
                                                              uint32_t actualSeqLen, uint32_t startChunk,
                                                              uint32_t chunkNum, uint32_t firstScanOffset,
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline bool LIVector<LIT, MM1_OUT_T>::FindEvictCandidates(uint32_t bIdx, uint32_t cacheRowIdx,
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline uint32_t LIVector<LIT, MM1_OUT_T>::CopyDecodedPayloadOut(
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline bool LIVector<LIT, MM1_OUT_T>::IsTopKIndex(const LocalTensor<int32_t> &indexLocal,
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline bool LIVector<LIT, MM1_OUT_T>::FindFallbackEvict(uint32_t cacheRowIdx, uint32_t actualSeqLen,
                                                        uint32_t cacheTokenCount,
                                                        const LocalTensor<int32_t> &indexLocal,
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::UpdateCacheAndWriteTopkSlots(
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::WriteMissCount(uint32_t bIdx, int32_t missCount,
                                                     const LocalTensor<int32_t> &scalarLocal)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::LoadKeyScaleChunk(const LICommon::RunInfo &info,
                                                                   const LocalTensor<half> &dst)
{
    uint64_t s2BaseOffset = static_cast<uint64_t>(info.s2Idx) * s2BaseSize_;
    uint64_t remaining = info.actualSingleProcessSInnerSize;
    ...
    while (remaining > 0) {
        ...
        uint64_t copyLen = Min(remaining, blockRemain);
        int32_t physicalBlock = blockTableGm_.GetValue(info.bIdx * maxBlockNumPerBatch_ + blockId);
        uint64_t gmOffset = static_cast<uint64_t>(physicalBlock) * blockSize_ + blockOffset;
        AscendC::DataCopyPad(dst[dstOffset], keyScaleGm[gmOffset],
                             AscendC::DataCopyExtParams{1, static_cast<uint32_t>(copyLen * sizeof(half)), 0, 0, 0},
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::ProcessVec0(const LICommon::RunInfo &info)
{
    if ((GetBlockIdx() & 1U) != 0) {
        return;
    }
    // Fold w * q_scale into the fp16 A operand of the cube-side head
    // reduction: gSize_ halfs, each broadcast to a full BLOCK_CUBE block
    // (Brcb), consumed by the cube WeightDmaCopy/LoadWeightToL0a pair.
    // Weights arrive as bf16 and there is no direct bf16 -> half Cast on this
    // arch, so the multiply is done in fp32 and rounded once to fp16.
    int64_t scaleGmOffset = static_cast<int64_t>(info.bIdx) * gSize_;
    LocalTensor<float> bufUb = inQueue_.AllocTensor<float>();
    LocalTensor<float> scaleFloatUb = bufUb;
    LocalTensor<float> weightFloatUb = bufUb[gSize_];
    LocalTensor<half> scaleHalfUb = bufUb[gSize_ * 2].template ReinterpretCast<half>();
    LocalTensor<bfloat16_t> weightBf16Ub = bufUb[gSize_ * 2 + gSize_ / 2].template ReinterpretCast<bfloat16_t>();
    LocalTensor<half> resHalfUb = bufUb[gSize_ * 3].template ReinterpretCast<half>();

    AscendC::DataCopyExtParams copyInParams{1, static_cast<uint32_t>(gSize_ * sizeof(half)), 0, 0, 0};
    AscendC::DataCopyPadExtParams<half> padParams{false, 0, 0, 0};
    AscendC::DataCopyPadExtParams<bfloat16_t> padTParams{false, 0, 0, 0};
    AscendC::DataCopyPad(scaleHalfUb, queryScaleGm[scaleGmOffset], copyInParams, padParams);
    AscendC::DataCopyPad(weightBf16Ub, weightsGm[scaleGmOffset], copyInParams, padTParams);
    SetWaitFlag<HardEvent::MTE2_V>(HardEvent::MTE2_V);

    AscendC::Cast(scaleFloatUb, scaleHalfUb, RoundMode::CAST_NONE, gSize_);
    AscendC::Cast(weightFloatUb, weightBf16Ub, RoundMode::CAST_NONE, gSize_);
    AscendC::PipeBarrier<PIPE_V>();
    AscendC::Mul(scaleFloatUb, scaleFloatUb, weightFloatUb, gSize_);
    AscendC::PipeBarrier<PIPE_V>();
    AscendC::Cast(resHalfUb, scaleFloatUb, RoundMode::CAST_RINT, gSize_);
    AscendC::PipeBarrier<PIPE_V>();

    LocalTensor<half> resUb = outQueue_.AllocTensor<half>();
    AscendC::Brcb(resUb, resHalfUb, static_cast<uint8_t>(gSize_ / B32_BLOCK_ALIGN_NUM), {1, B32_VEC_REPEAT_STRIDE});
    inQueue_.FreeTensor(bufUb);
    SetWaitFlag<HardEvent::V_MTE3>(HardEvent::V_MTE3);
    AscendC::DataCopyPad(weightWorkspaceGm[(info.loop % 2) * (gSize_ * BLOCK_CUBE)], resUb,
                         AscendC::DataCopyExtParams{1, static_cast<uint32_t>(gSize_ * BLOCK_CUBE * sizeof(half)),
                                                    0, 0, 0});
    SetWaitFlag<HardEvent::MTE3_V>(HardEvent::MTE3_V);
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::StoreMtpQueryTopK(const LICommon::RunInfo &info)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::FinalizeMtpRequest(const LICommon::RunInfo &info)
{
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::ProcessVecMtp(const LICommon::RunInfo &info)
{
    if ((GetBlockIdx() & 1U) != 0U) {
        return;
    }
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::ProcessVec(const LICommon::RunInfo &info)
{
    if ((GetBlockIdx() & 1U) != 0) {
    return;
    }
    ...
    int32_t cuBaseS2Idx = info.s2Idx * s2BaseSize_;
    int32_t cuS2Len = info.actualSingleProcessSInnerSize;
    // C8: one fp32 score row per chunk buffer; bf16: qHeadNum rows per buffer.
    int64_t mmGmOffset = LIT::IS_C8 ? (info.loop % 2) * s2BaseSize_
                                    : (info.loop % 2) * (gSize_ * s2BaseSize_);
    ...
    LocalTensor<half> keyScaleHalfUb;
    LocalTensor<float> keyScaleFloatUb;
    if constexpr (LIT::IS_C8) {
        keyScaleHalfUb = keyScaleBuf_.Get<half>();
        keyScaleFloatUb = keyScaleBuf_.Get<half>()[S2_BASE_SIZE].template ReinterpretCast<float>();
        LoadKeyScaleChunk(info, keyScaleHalfUb);
        AscendC::Cast(keyScaleFloatUb, keyScaleHalfUb, RoundMode::CAST_NONE, info.actualSingleProcessSInnerSizeAlign);
        AscendC::PipeBarrier<PIPE_V>();
    }

    if constexpr (LIT::IS_C8) {
        // Cube already did dequant + head reduction: mm1ResGm holds the final
        // fp32 score row for this chunk (QLI-style two-mma flow). Only the
        // per-token k_scale multiply remains before sort/topk.
        ...
        AscendC::DataCopyPad(reduceOutInner, mm1ResGm[mmGmOffset], scoreCopyParams, scorePadParams);
        SetWaitFlag<HardEvent::MTE2_V>(HardEvent::MTE2_V);
...
template <typename LIT, typename MM1_OUT_T>
__aicore__ inline void LIVector<LIT, MM1_OUT_T>::FinalizePartialRequest(uint32_t bIdx, uint32_t cacheRowIdx,
                                                             uint32_t actualSeqLen,
                                                             uint32_t cacheTokenCount,
                                                             uint32_t firstOwner, uint32_t lastOwner)
{
