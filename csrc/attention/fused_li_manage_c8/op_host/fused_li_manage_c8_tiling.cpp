...
#include "fused_li_manage_c8_tiling.h"
#include <algorithm>
#include "../op_kernel/fused_li_manage_c8_template_tiling_key.h"
...
ge::graphStatus FusedLiManageC8Tiling::GetNpuInfo(FusedLiManageC8TilingInfo &tilingInfo) const
{
    if (context_->GetNodeName() == nullptr) {
        OPS_LOG_E("FusedLiManageC8", "opName got from TilingContext is nullptr.");
...
ge::graphStatus FusedLiManageC8Tiling::GetTensorInfo(FusedLiManageC8TilingInfo &tilingInfo) const
{
    ...
    op.queryScale.desc = context_->GetInputDesc(QUERY_SCALE_INDEX);
    op.queryScale.shape = context_->GetInputShape(QUERY_SCALE_INDEX);
    op.keyScaleCache.desc = context_->GetInputDesc(KEY_SCALE_CACHE_INDEX);
    op.keyScaleCache.shape = context_->GetInputShape(KEY_SCALE_CACHE_INDEX);
    ...
    op.numCandidateTokens.desc = context_->GetInputDesc(NUM_CANDIDATE_TOKENS_INDEX);
    op.numCandidateTokens.tensor = context_->GetInputTensor(NUM_CANDIDATE_TOKENS_INDEX);
    op.numCacheTokens.desc = context_->GetInputDesc(NUM_CACHE_TOKENS_INDEX);
    op.numCacheTokens.tensor = context_->GetInputTensor(NUM_CACHE_TOKENS_INDEX);
    ...
    OPS_ERR_IF(op.queryScale.desc == nullptr || op.queryScale.shape == nullptr,
               OPS_LOG_E(tilingInfo.opName, "query_scale desc/shape is nullptr."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(op.keyScaleCache.desc == nullptr || op.keyScaleCache.shape == nullptr,
               OPS_LOG_E(tilingInfo.opName, "key_scale_cache desc/shape is nullptr."), return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(op.numCandidateTokens.desc == nullptr || op.numCandidateTokens.tensor == nullptr,
               OPS_LOG_E(tilingInfo.opName, "num_candidate_tokens desc/tensor is nullptr."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(op.numCacheTokens.desc == nullptr || op.numCacheTokens.tensor == nullptr,
               OPS_LOG_E(tilingInfo.opName, "num_cache_tokens desc/tensor is nullptr."), return ge::GRAPH_FAILED);
...
ge::graphStatus FusedLiManageC8Tiling::CheckDtype(const FusedLiManageC8TilingInfo &tilingInfo) const
{
    const auto &op = tilingInfo.opParamInfo;
    ge::DataType qType = op.query.desc->GetDataType();
    ge::DataType kType = op.key.desc->GetDataType();
    ge::DataType qsType = op.queryScale.desc->GetDataType();
    ge::DataType ksType = op.keyScaleCache.desc->GetDataType();
    ge::DataType wType = op.weights.desc->GetDataType();
    OPS_ERR_IF(qType != ge::DT_INT8,
               OPS_LOG_E(tilingInfo.opName, "query dtype must be int8."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(kType != ge::DT_INT8,
               OPS_LOG_E(tilingInfo.opName, "key dtype must be int8."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(qsType != ge::DT_FLOAT16,
               OPS_LOG_E(tilingInfo.opName, "query_scale dtype must be fp16."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(ksType != ge::DT_FLOAT16,
               OPS_LOG_E(tilingInfo.opName, "key_scale_cache dtype must be fp16."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(wType != ge::DT_FLOAT16 && wType != ge::DT_BF16,
               OPS_LOG_E(tilingInfo.opName, "weights dtype must be fp16 or bf16."),
               return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(op.numCandidateTokens.desc->GetDataType() != ge::DT_INT32,
               OPS_LOG_E(tilingInfo.opName, "num_candidate_tokens dtype must be int32."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(op.numCacheTokens.desc->GetDataType() != ge::DT_INT32,
               OPS_LOG_E(tilingInfo.opName, "num_cache_tokens dtype must be int32."), return ge::GRAPH_FAILED);
...
ge::graphStatus FusedLiManageC8Tiling::CheckShape(FusedLiManageC8TilingInfo &tilingInfo) const
{
    const auto &op = tilingInfo.opParamInfo;
    ...
    const auto &queryScaleShape = op.queryScale.shape->GetStorageShape();
    const auto &keyScaleShape = op.keyScaleCache.shape->GetStorageShape();
    ...
    const auto &numCandidateShape = op.numCandidateTokens.tensor->GetStorageShape();
    const auto &numCacheShape = op.numCacheTokens.tensor->GetStorageShape();
    ...
    OPS_ERR_IF(numCandidateShape.GetDimNum() != DIM_NUM_ONE,
               OPS_LOG_E(tilingInfo.opName, "num_candidate_tokens must be rank 1."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(numCacheShape.GetDimNum() != DIM_NUM_ONE,
               OPS_LOG_E(tilingInfo.opName, "num_cache_tokens must be rank 1."), return ge::GRAPH_FAILED);
    ...
    tilingInfo.keyBlockNum = static_cast<uint32_t>(kShape.GetDim(0));
    tilingInfo.keyScaleCount = tilingInfo.keyBlockNum * tilingInfo.blockSize;
    tilingInfo.s2Size = 0;
    ...
    OPS_ERR_IF(reqPoolShape.GetShapeSize() != tilingInfo.bSize ||
                   numCandidateShape.GetShapeSize() != tilingInfo.bSize ||
                   numCacheShape.GetShapeSize() != tilingInfo.bSize ||
                   blockShape.GetDim(0) != tilingInfo.bSize,
               OPS_LOG_E(tilingInfo.opName,
                         "query batch, req_pool_entries, num_candidate_tokens, num_cache_tokens,"
                         " and block_table batch must match."),
               return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(queryScaleShape.GetDimNum() != DIM_NUM_TWO ||
                   queryScaleShape.GetDim(0) != tilingInfo.bSize ||
                   queryScaleShape.GetDim(1) != tilingInfo.n1Size,
               OPS_LOG_E(tilingInfo.opName, "query_scale must be [B, N1]."),
               return ge::GRAPH_FAILED);
    OPS_ERR_IF(keyScaleShape.GetDimNum() != DIM_NUM_FOUR ||
                   keyScaleShape.GetDim(0) != kShape.GetDim(0) ||
                   keyScaleShape.GetDim(1) != kShape.GetDim(1) ||
                   keyScaleShape.GetDim(DIM_IDX_TWO) != 1 ||
                   keyScaleShape.GetDim(DIM_IDX_THREE) != 1,
               OPS_LOG_E(tilingInfo.opName,
                         "key_scale_cache must be [num_blocks, block_size, 1, 1] matching key."),
               return ge::GRAPH_FAILED);
...
ge::graphStatus FusedLiManageC8Tiling::ParseAndCheck(FusedLiManageC8TilingInfo &tilingInfo)
{
...
ge::graphStatus FusedLiManageC8Tiling::DoTiling(FusedLiManageC8TilingInfo *tilingInfo)
{
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(tilingInfo->platformInfo);
    uint32_t aivNum = ascendcPlatform.GetCoreNumAiv();
    uint32_t aicNum = ascendcPlatform.GetCoreNumAic();
    ...
    constexpr uint32_t WEIGHT_BLOCK_CUBE = 16;
    uint64_t workspaceSize = ascendcPlatform.GetLibApiWorkSpaceSize();
    // Cube-side head reduction (QLI-style): mm1Res holds one fp32 score row
    // per chunk buffer instead of qHeadNum int32 rows.
    workspaceSize += static_cast<uint64_t>(S2_BASE_SIZE) * MM1_RES_ELEM_SIZE * DOUBLE_BUFFER * blockDim;
    // Brcb'd (w*q_scale) fp16 mma2 operand, double buffered per core.
    workspaceSize += static_cast<uint64_t>(tilingInfo->n1Size) * WEIGHT_BLOCK_CUBE * sizeof(uint16_t) *
                     DOUBLE_BUFFER * blockDim;
    ...
    tilingData_.set_keyScaleCount(tilingInfo->keyScaleCount);
    tilingData_.set_usedCoreNum(blockDim);
    tilingData_.set_scheduleMode(selectedScheduleMode);
    tilingData_.SaveToBuffer(context_->GetRawTilingData()->GetData(), context_->GetRawTilingData()->GetCapacity());
    context_->GetRawTilingData()->SetDataSize(tilingData_.GetDataSize());

    uint32_t tilingKey = GET_TPL_TILING_KEY(LI_TPL_C8);
    context_->SetTilingKey(tilingKey);
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus TilingPrepareForFusedLiManageC8(gert::TilingParseContext * /* context */)
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus TilingForFusedLiManageC8(gert::TilingContext *context)
{
    OPS_ERR_IF(context == nullptr,
               OPS_REPORT_VECTOR_INNER_ERR("FusedLiManageC8", "Tiling context is null."),
               return ge::GRAPH_FAILED);
    FusedLiManageC8TilingInfo liInfo;
    FusedLiManageC8Tiling liTiling(context);
...
IMPL_OP_OPTILING(FusedLiManageC8)
    .Tiling(TilingForFusedLiManageC8)
    .TilingParse<FusedLiManageC8CompileInfo>(TilingPrepareForFusedLiManageC8);
