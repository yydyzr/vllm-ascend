...
except ImportError:  # pragma: no cover - older vLLM without hybrid Mamba specs
    MambaSpec = None  # type: ignore[misc, assignment]

try:
    from vllm.v1.structured_output import StructuredOutputGrammar
except ImportError:  # pragma: no cover - older vLLM structured-output layout
    StructuredOutputGrammar = None  # type: ignore[misc, assignment]
...
class RecomputeScheduler(Scheduler):
    ...
    def _has_pending_deliverable_output(self, request: Request) -> bool:
        return getattr(request, "num_stale_output_tokens", 0) > 0 and not getattr(request, "drop_stale_output", False)
    ...
    def _truncate_computed_blocks_for_connector(
        ...
        kv_cache_manager = self.kv_cache_manager
        truncate_blocks = getattr(kv_cache_manager, "truncate_computed_blocks", None)
        for group_blocks, manager, group in zip(
            ...
            if MambaSpec is not None and isinstance(group.kv_cache_spec, MambaSpec) and num_blocks > len(group_blocks):
                return self._truncate_computed_blocks_with_mamba_clamp(blocks, num_computed_tokens)
        if callable(truncate_blocks):
            return truncate_blocks(blocks, num_computed_tokens)
        return self._truncate_computed_blocks_with_mamba_clamp(blocks, num_computed_tokens)
    ...
    def _truncate_computed_blocks_with_mamba_clamp(
        ...
        for group_blocks, manager, group in zip(
            ...
            num_blocks = num_computed_tokens // manager.block_size
            if MambaSpec is not None and isinstance(group.kv_cache_spec, MambaSpec):
        ...
        return kv_cache_manager.create_kv_cache_blocks(tuple(truncated))
    ...
    def _preempt_request(
        ...
        parent = super()._preempt_request
        if "drop_stale_output" in inspect.signature(parent).parameters:
            parent(request, timestamp, drop_stale_output=drop_stale_output)
        else:
    ...
    def _update_waiting_for_remote_kv(self, request: Request) -> None:
        ...
        When the kv transfer is ready, cache the blocks and move the request
        back to WAITING from WAITING_FOR_REMOTE_KV.
        """
    ...
    def schedule(self, throttle_prefills: bool = False) -> RecomputeSchedulerOutput:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if request.num_computed_tokens == 0:
                    ...
                    if self.connector is not None:
                        ...
                        supports_partial_tail = callable(
                            getattr(self.kv_cache_manager, "truncate_computed_blocks", None)
                        )
                        ...
                        if partial_tail and ext_tokens > partial_tail:
                            new_computed_blocks = self._truncate_computed_blocks_for_connector(
        ...
        if self.log_stats and getattr(self.observability_config, "enable_logging_iteration_details", False):
            make_encoder_stats = getattr(self, "_make_scheduled_encoder_input_stats", None)
            if callable(make_encoder_stats):
                scheduled_encoder_input_stats = make_encoder_stats(scheduled_encoder_inputs)
        ...
        if self._scheduler_output_supports("scheduled_encoder_input_stats"):
            scheduler_output_kwargs["scheduled_encoder_input_stats"] = scheduled_encoder_input_stats
    ...
    def update_from_output(
        ...
        update_output_supports_stale = "is_stale" in inspect.signature(self._update_request_with_output).parameters
        should_advance_supports_new_tokens = (
            "new_token_ids" in inspect.signature(self.structured_output_manager.should_advance).parameters
        )
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            if request is not None:
                ...
                stale_tokens = getattr(request, "num_stale_output_tokens", 0)
                if stale_tokens > 0:
            ...
            if output_is_stale and getattr(request, "drop_stale_output", False):
                continue
            ...
            if new_token_ids:
                if update_output_supports_stale:
            ...
            if new_token_ids:
                if should_advance_supports_new_tokens:
                    should_advance = self.structured_output_manager.should_advance(request, new_token_ids=new_token_ids)
                else:
            ...
            if should_advance:
                ...
                if StructuredOutputGrammar is not None:
            ...
            if should_emit_output:
                ...
                if prefill_stats is not None and hasattr(prefill_stats, "finalize"):
                    estimate_cached = getattr(self.kv_cache_manager, "estimate_cached_tokens", None)
                    if callable(estimate_cached):
                        prefill_stats.finalize(estimate_cached(request))
        ...
        error_req_ids = set(getattr(self, "grammar_compile_error_reqs", ()))
        if hasattr(self, "grammar_compile_error_reqs"):
